...
function install_nodes() {
    ...
    if [ "$NODE_COUNT" -eq 0 ]; then
        echo "❌ Не введено жодного ключа. Вихід."
        return
    fi
    echo "🔹 Ви ввели $NODE_COUNT ключ(ів)."

    read -p "Введіть загальний обсяг оперативної пам'яті для нод (в МБ, наприклад 6144): " TOTAL_MEM_MB
    if ! [[ "$TOTAL_MEM_MB" =~ ^[0-9]+$ ]]; then
        echo "❌ Некоректне значення пам'яті."
        return
    fi

    MEMORY_LIMIT_MB=$(( TOTAL_MEM_MB / NODE_COUNT ))
    if [ "$MEMORY_LIMIT_MB" -lt 100 ]; then
        echo "❌ Пам'яті на ноду замало (менше 100 МБ). Збільшіть загальний обсяг або зменшіть кількість нод."
        return
    fi

    MEMORY_LIMIT="${MEMORY_LIMIT_MB}m"
    echo "🔹 Пам'ять на одну ноду: $MEMORY_LIMIT"
    ...
    for (( i=0; i<NODE_COUNT; i++ )); do
        ...
        echo "🔹 Запуск контейнера $CONTAINER_NAME з портом $PORT та обмеженням пам'яті $MEMORY_LIMIT"

        if ! docker run -d --name "$CONTAINER_NAME" \
            -e DATAGRAM_KEY="$NODE_KEY" \
            -p "$PORT:5000" \
            --memory "$MEMORY_LIMIT" \
